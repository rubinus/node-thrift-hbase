    ...
    return poolModule.Pool({
        ...
        create: function (callback) {
            ...
            if (options.hosts.length > 0) {
                ...
                var host = options.hosts[Math.floor(Math.random() * options.hosts.length)];
            }
            var clientOption = {port: options.port, host: host , timeout: options.timeout};
            ...
            client.connection.on('timeout', function (err) {
                console.log('getClient timeout', err);
                callback(err);
            });
        },
        ...
        max: options.minConnections || 10,
        idleTimeoutMillis: options || 3600000
    });
};
...
var Client = function (options) {
    ...
    var connection = thrift.createConnection(this.host, this.port, {timeout: options.timeout || 0});
